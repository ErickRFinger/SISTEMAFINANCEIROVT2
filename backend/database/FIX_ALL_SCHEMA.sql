-- =================================================================
-- SCRIPT "BALA DE PRATA" (FIX COMPLETE V1)
-- DATA: 2026-01-05
-- OBJETIVO: Resetar tabelas empresariais + Garantir UUID + Liberar Permissões
-- =================================================================

BEGIN;

-- 1. LIMPEZA (Apaga tabelas antigas se existirem)
...
-- 2. RECRIAR TABELAS (Schema compatível com UUID do Auth)

-- Tabela Funcionários
CREATE TABLE public.funcionarios (
    id SERIAL PRIMARY KEY,
    user_id UUID NOT NULL,               -- OBRIGATÓRIO: UUID (não BIGINT)
...
CREATE TABLE public.clientes (
    id SERIAL PRIMARY KEY,
    user_id UUID NOT NULL,               -- OBRIGATÓRIO: UUID
    nome VARCHAR(255) NOT NULL,
    email VARCHAR(255),
    telefone VARCHAR(20),
...
-- 3. FUNÇÃO AUXILIAR (Kanban Reorder)
...
-- 4. PERMISSÕES "TUDO LIBERADO" (Para evitar erro 403 Forbidden)
-- Desabilita RLS (Row Level Security) para deixar o backend filtrar via user_id
...
-- Garante que o usuario anon/service pode ler/escrever (redundancia de seguranca)
